...
def draw_legacy_stars(ax, pct, fig_sz):
    ...
    for el in ECOLOGY_ELEMENTS:
        if el['death'] > pct:
            continue          # still alive or not yet born
        pos = ECOLOGY_POSITIONS.get(el['name'])
        if pos is None:
            continue
        ex, ey = pos
        col  = el.get('color', el.get('bk', '#ffffff'))  # star→color, neb→bk
        age  = min(1.0, (pct - el['death']) / 25.0)   # 0=just died, 1=long faded
...
def epoch_intensities(pct):
    return {
        'domain_i': [lerp(D0_DARK_MARK,pct),lerp(D1_PROTECT,pct),lerp(D2_FRIEND,pct),
                     lerp(D3_DESTINY,pct),lerp(D4_LONELY,pct)],
        'rel_i':    [lerp(R0_HERMIONE,pct),lerp(R1_RON,pct),lerp(R2_VOLDEMORT,pct),
                     lerp(R3_DUMBLEDORE,pct),lerp(R4_LILY,pct)],
        'pat_d':    [lerp(P0_LEO,pct),lerp(P1_SCORPIUS,pct),
...
FIXED_POS = precompute_positions(HARRY_UNIVERSE)
...
ECOLOGY_ELEMENTS = [
    ...
    # Nebulae  (excl_r >= max(rx,ry) to prevent visual overlap)
...
_N_ANIM = 60
ECOLOGY_POSITIONS = precompute_ecology_positions(HARRY_UNIVERSE, _N_ANIM)
...
def render_frame(char, fixed_pos, pct, frame_idx, output_path,
    ...
    # ── Voldemort intensity annotation (upper-left) ───────────────────
    voldemort_v = lerp(R2_VOLDEMORT, pct)
    voldemort_tier = ('★恒星' if voldemort_v>=REL_THRESH else
                      '◉云' if voldemort_v>=4.5 else
                      '·碎片' if voldemort_v>=1.5 else '消散')
    ax.text(0.4, H-0.5,
           f"伏地魔  {voldemort_v:.1f}  [{voldemort_tier}]",
           fontsize=6.5,color='#c040ff',alpha=0.55,
...
def main():
    N = _N_ANIM   # 60 frames
    print("═"*62)
    print("  Soul Star Map  v28  — Harry Potter Full Lifecycle Animation")
    print(f"  {N} frames × 100 sentences = 哈利·波特全程心路历程")
    print("  所有星体位置锁定 — 伏地魔始终在左上角")
    print("  伏地魔弧线: 碎片→原始云→恒星→顶峰→消散→碎片")
    print("  v28新增: 生态元素 + 天空大气层")
    print(f"  Output: {OUT}/")
    print("═"*62)

    # Verify Voldemort fixed position
    vol_x, vol_y = FIXED_POS['伏地魔']
    print(f"\n  伏地魔固定位置: ({vol_x:.2f}, {vol_y:.2f})  "
          f"← 左{vol_x/W*100:.0f}%  上{vol_y/H*100:.0f}%")
    ...
    for fi in range(N):
        ...
        char = build_epoch(HARRY_UNIVERSE, intensities, ecology_pct=pct)

        voldemort_v = lerp(R2_VOLDEMORT, pct)
        beat_name, _ = get_beat(pct)
        tier = ('★恒星' if voldemort_v>=REL_THRESH else
                '◉云' if voldemort_v>=4.5 else '·碎片')
        ...
        print(f"  [{fi:02d}/{N-1}] {pct:5.1f}% | {beat_name:4s} | "
              ...
              f"Voldemort={voldemort_v:.1f}{tier} | {elapsed:.1f}s")
        frame_paths.append(out)

    # GIF
    gif_path = OUT / "Harry_Potter_灵魂星图.gif"
    print(f"\n  合成GIF ({N}帧 × 300ms = {N*300/1000:.1f}秒)…")
    make_gif(frame_paths, gif_path, duration_ms=300)
    print(f"  → {gif_path.name}")
    print(f"\n  Total: {(time.time()-t0)/60:.1f} min")
    print(f"\n  伏地魔生命轨迹验证:")
    for pct_ in [0,14,30,50,65,78,95,100]:
        v = lerp(R2_VOLDEMORT, pct_)
